...
description: >
  Maintains persistent state files including timeline, character states, location states, and unresolved thread ledger for cross-session state management.
license: GPL-3.0
...
**Triggers:** "update state", "track timeline", "log character status", "update timeline", "track threads"
...
1. Load the continuity-tracker sub-skill and review the reference templates in method-patterns-continuity.md
2. Initialize state files: timeline.md, character-states.md, location-states.md, thread-ledger.md
3. Log events and state changes as they occur during writing sessions
4. Create state snapshots before major plot developments or rewrites
5. Maintain thread ledger to track unresolved plot threads across sessions
...
- [example-chapter-draft.md](../../references/examples/example-chapter-draft.md) - Worked example chapter whose events are recorded in state files. For timeline, character-states, and thread-ledger templates, see [method-patterns-continuity.md](../../references/method-patterns-continuity.md) Sections 2-5.

## Quick Reference: File Conventions

- **Naming:** State files use YAML frontmatter for machine readability. Timeline entries use ISO 8601 dates. Thread ledger tracks status: open / resolved / abandoned.
- **Encoding:** UTF-8
...
1. [ ] Timeline entries are in chronological order
2. [ ] Character state updates are dated and attributed
3. [ ] Thread ledger entries have clear resolution criteria
4. [ ] State snapshots are created before major changes
5. [ ] No orphaned references (state pointing to non-existent entities)
6. [ ] File encoding is UTF-8 throughout
...
Scope: Maintains persistent state files including timeline, character states, location states, and unresolved thread ledger for cross-session state management.
