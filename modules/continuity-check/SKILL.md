...
description: >
  Verifies consistency across all chapters and state files, identifying plot holes, timeline errors, and character inconsistency.
license: GPL-3.0
...
**Triggers:** "check continuity", "verify consistency", "find plot holes", "continuity audit", "consistency check"
...
1. Load the continuity-check sub-skill and review the reference templates in method-patterns-continuity.md
2. Gather all chapters, world-building docs, character sheets, and state files
3. Run multi-dimensional consistency checks: names, timeline, locations, plot threads, rules
4. Generate structured continuity report with severity levels (Critical / Warning / Suggestion)
5. File report and feed findings back to chapter-writing or character-design modules for fixes
...
- [Method Patterns](../../references/method-patterns-continuity.md) - Master template file with structured patterns and guidelines
...
- [example-chapter-draft.md](../../references/examples/example-chapter-draft.md) - Worked example chapter showing the continuity expectations this module verifies. For the continuity check report format (Critical / Warning / Suggestion), see [method-patterns-continuity.md](../../references/method-patterns-continuity.md) Section 6.

## Quick Reference: File Conventions

- **Naming:** Continuity reports use structured format with severity levels: Critical / Warning / Suggestion. Reports are saved as `state/continuity-reports/{chapter-or-date}.md`.
  Do NOT write them to `drafts/` — `.gitignore` excludes that directory, so reports placed there are never committed and the audit trail is lost.
- **Encoding:** UTF-8
...
1. [ ] All character names spelled consistently throughout
2. [ ] Timeline events do not contradict established chronology
3. [ ] Location descriptions match previous appearances
4. [ ] Plot threads are either resolved or explicitly left open
5. [ ] No unexplained knowledge gaps (characters knowing things they should not)
6. [ ] Magic/technology rules applied consistently
7. [ ] Time references (days, seasons, ages) are consistent
...
Scope: Verifies consistency across all chapters and state files, identifying plot holes, timeline errors, and character inconsistency.
